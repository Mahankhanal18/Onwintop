...
if ($channel_id) {
    $method = 'create-video';
    $parts=URL_Parts();
    if(in_array('edit-video',$parts)){
        $method='edit-video';
    }
    if ($method == 'create-video') {
        //saving file
        if (isset($_POST['name'])) {
    ...
    } else if ($method == 'edit-video') {
        $file_id = $channel_id;
        $con = R::findOne("videos", "WHERE id=?", [$file_id]);
        $channel_id = $con['channel_id'];
        $edit_mode = true;
        if (isset($_POST['name'])) {
            $data = R::findOne("videos","WHERE id=?",[$file_id]);
            $data->name = $_POST['name'];
            $data->type = "Video";
            $data->category = $_POST['categories'];
            $data->url = $_POST['file'];
            ...
            R::store($data);
            //creating content
            $content = R::findOne("contents","WHERE data_id=? AND type=?",[$data['id'],'Video']);
            $content->name = $_POST['name'];
            $content->type = "Video";
            $content->tags = $_POST['categories'];
            $content->thumbnail = $_POST['thumbnail'];
...
<head>
    <title><?php if($edit_mode) echo "Edit Video"; else echo "Create Video";?> | <?php echo $title; ?></title>
                            ...
                            <div id="page-contents" class="row merged20">
                                <div class="col-lg-12">
                                    <div class="main-wraper">
                                        <div class="main-title"><?php if($edit_mode) echo "Edit Video"; else echo "Create Video";?>
                                        </div>
                                            ...
                                            <form id='create-data' action='' method='post' class="c-form">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-4">
                                                        ...
                                                        <input type='file' id='file' accept='.mp4,.mov,.3gp' style='display:none' />
